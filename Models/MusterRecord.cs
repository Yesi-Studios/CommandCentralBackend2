...
namespace CommandCentral.Models
{
    ...
    public class MusterRecord
    {
        ...
        /// </summary>
        public static Time RolloverTime { get; } = new Time(20, 0, 0);
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Musterer);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Musteree);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Paygrade);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Designation);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(UIC);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Division);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Department);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Command);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(MusterStatus);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(DutyStatus);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(SubmitTime);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(MusterDate);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(HasBeenSubmitted);
                hash = hash * 23 + NullSafeUtilities.GetSafeHashCode(Remarks);
        ...
        {
            //TODO Log.Info("Detecting current muster state...");
            ...
            using (var transaction = session.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        //TODO Log.Info("Correcting {0} persons with incorrect muster records...".With(personsWithIncorrectRecords.Count));
                        ...
                        foreach (var person in personsWithIncorrectRecords)
                        {
                            ...
                            if (current % 2 == 0)
                            {
                                //TODO Log.Info("Fixed {0}% of muster records...".With(Math.Round(((double)current / (double)personsWithIncorrectRecords.Count) * 100, 2)));
                            }

                            session.Save(person);
                        }

                        //TODO Log.Info("Muster record clean up completed.");
                    }

                    //Ok, at this point, we know that we have muster records for today.  Let's just tell the host how far along we are.
                    //TODO Log.Info("{0}/{1} person(s) have been mustered so far.".With(persons.Count(x => x.CurrentMusterRecord.HasBeenSubmitted), persons.Count));
                    //TODO Log.Info("Muster finalization status : {0}".With(MusterRecord.IsMusterFinalized ? "Finalized" : "Not Finalized"));
                    //TODO Log.Info("Rollover time : {0}".With(MusterRecord.RolloverTime));
                    //TODO Log.Info("Current muster date is: {0}".With(GetMusterDate(DateTime.UtcNow).ToString("D")));

                    //TODO Log.Info("Registering muster roll over to occur every day at '{0}'".With(MusterRecord.RolloverTime));
